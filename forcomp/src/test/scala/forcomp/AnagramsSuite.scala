...
class AnagramsSuite {
  ...
  @Test def `sentenceOccurrences: empty (5pts)`: Unit =
  ...
  @Test def `sentenceOccurrences2: abcd ea (5pts)`: Unit =
    assertEquals(List(('a', 2), ('b', 1), ('c', 1), ('d', 1), ('e', 1)), sentenceOccurrences(List("abcd", "ea")))
  ...
  @Test def `wordAnagrams Easter (2pts)`: Unit =
    assertEquals(Set("easter", "eaters", "Teresa"), wordAnagrams("Easter").toSet)
  ...
  @Test def `combinations: bb (8pts)`: Unit = {
    val abba = List(('b', 2))
    val abbacomb = List(
      List(),
      List(('b', 1)),
  ...
  @Test def `sentence from Easter`: Unit = {
    val sentence = List("me", "to")
    val anas = List(List("me", "to"), List("to", "me"), List("to", "em"), List("em", "to"))

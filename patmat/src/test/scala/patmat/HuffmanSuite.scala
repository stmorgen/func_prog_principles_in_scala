...
class HuffmanSuite {
  ...
  @Test def `times`: Unit =
    new TestTrees {
      assertEquals(List(('a', 2), ('b',1)), Huffman.times(List('a','b','a')))
    }

  @Test def `decodedSecret`: Unit =
    new TestTrees {
      assertEquals("abc", Huffman.decodedSecret)
    }

  @Test def `minimal`: Unit =
    new TestTrees {

      val tree = Huffman.createCodeTree(List('s', 'o', 'm', 'e', 'T', 'e', 'x', 't'))

      assertEquals("abc", Huffman.quickEncode(tree)(Huffman.string2Chars("someText")))
    }

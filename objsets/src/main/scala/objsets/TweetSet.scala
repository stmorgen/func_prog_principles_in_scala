...
class Empty extends TweetSet {
  ...
  override def mostRetweeted: Tweet = throw new NoSuchElementException("No Max element in empty set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val x = this.elem
    if (p(x)) left.filterAcc(p, acc) union new Empty().incl(x) union right.filterAcc(p, acc)
    else left.filterAcc(p, acc) union right.filterAcc(p, acc)
  }

  def union (that: TweetSet): TweetSet = new NonEmpty(this.elem, left union right, that)
  ...
  override def mostRetweeted: Tweet = {
    ...
    if(left.isEmpty() && right.isEmpty()) elem
      ...
      val maxElem = (left union right).mostRetweeted
      if (maxElem.retweets > elem.retweets) maxElem
      else elem
    }
  }
